/*
набор примеров, иллюстрирующих основные концепции и инструменты параллельного программирования:
- Создание потоков, функции потоков, передача данных в поток
- Получение данных из потока, будущие значения (future) передача данных из потока через параметр-ссылку
- Потокобезопасность
- Неопределённость параллелизма (состояние гонки) при неатомарных операциях изменения общих переменных
- Синхронизация через мьютексы; критические секции
...
void example1_1_bad(){
    ...
    // при завершении текущей функции будет уничтожен объект th1,
...
//    example1_1_bad();  // Как не нужно создавать поток в локальной области видимости
