"""
Простой пример асинхронной программы из 3 функций возвращающих значение

Три асинхронных функции, каздая из которых работает 1, 2 и 3 секунды выполняются суммарно за 3 с небольшим секунды.
Это возможно за счёт того, что функции позволяют выполняться друг другу, пока каждая из них ожидает событие (окончание таймераs)
"""
...
async def main_await_gather():
    ...
    results = await asyncio.gather( c1(), c2(), c3())     # поставить корутины в очередь на выполнение и начать выполнять
    # здесь будет происходить неблокирующее ожидание
    print("Results: ", end="")
    print(results)
...
start = time.time()
asyncio.run( main_await_gather() )       # запуск цикл обработки событий (event loop) и главную функцию с корутинами
