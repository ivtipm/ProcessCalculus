...
Пример: синхронные и асинхроннеы запросы
...
2. Цикл по очереди обрабатывает _задачи_ из очереди задач (Tast Queue)
3. Обработка задачи:
   ...
   7. Цикл обработки сообщий проверяет: нет ли корутины, которая готова продолжить выполнение (check for computed IO tasks).
...
- `results = await asyncio.gather(coro1(), coro2())` – ожидание звершения нескольких корутин
