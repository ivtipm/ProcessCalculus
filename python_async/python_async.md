...
* Используются корутины (`async def fetch_data()`) для имитации длительных операций.
* Ключевое слово `await` позволяет приостановить выполнение до завершения `asyncio.sleep`.
* `asyncio.gather` позволяет запускать несколько задач параллельно (в рамках одного потока).

---

## Заключение

Библиотека **asyncio** предоставляет мощный инструмент для реализации асинхронного программирования, особенно когда речь идёт о задачах ввода-вывода. Благодаря концепциям event loop, корутин, task и Future, она позволяет создавать высокопроизводительные и масштабируемые приложения, обходя сложности и накладные расходы традиционных потоков или процессов в условиях ограничений GIL.

Эта шпаргалка дает общее представление об основных идеях и механизмах asyncio, что позволит быстрее освоить асинхронное программирование в Python.
