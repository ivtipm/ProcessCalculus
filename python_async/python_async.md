...
* Что такое цикл обработки событий в программах с GUI?
* Как влияет на работу программы долгое выполнение одного из обработчиков событий?
...
Асинхронность имеет смысл только тогда, когда вызываемая задача выполняет долго не из-за того, что она требует много ресурсов, а из-за того, что она требует ожидания. Например при вводе\выводе или обращении к сети. В то время, как вызванная задача (func1 на рис. ниже) ожидает, например, отклика по сети, другие задачи (func2) могут выполняться. Задачи всё ещё конкурируют за возможность выполняться, но одна функция даёт другой время для выполнения, если первая функция вынуждена простаивать.
...
Принцип работы цикла обработки событий (см. рис выше)

1. Программа запускает цикл обработки событий (event loop)
2. Цикл по очереди обрабатывает _задачи_ из очереди задач (Task Queue)
3. Обработка задачи:
   1. Из задачи "извлекается" корутина.
   2. Корутина начинает выполняться. Она не может быть приостановлена в произвольном месте кроме тех, где есть вызовы других задач через `await`.
   3. В месте вызова с `await` корутина позволяет себя приостановить, давая возможность выполняться другим задачам.
   4. `await` инициирует IO запрос. Предполагается, что ответ на этот запрос не будет получен мгновенно. Вместо результата запроса возвращается специальный объект (awaitable) объект.
   5. `await` приостанавливает корутину.
   6. Цикл обработки событий регистрирует callback для довыполнения этой корутины.
   7. Цикл обработки сообщений проверяет: нет ли корутины, которая готова продолжить выполнение (check for computed IO tasks).
      - Если такие корутина есть, то цикл обработки событий будет выполнять её.
      - Если таких корутин нет, то цикл проверяет очередь задач. Если задачи есть, то переход к п 3.1
      - Если нет, то переход к 3.7
...
**_CPU-bound задача_** — задача скорость выполнения которой ограничена в первую очередь скоростью работы процессора.
...
- **await** – ключевое слово, позволяющее приостановить выполнение корутины до получения результата асинхронной операции. `await` записывается перед _вызовом_ функции. Например `res = await httpx.get( ... )`
...
- `results = await asyncio.gather(coro1(), coro2())` – ожидание завершения нескольких корутин
- `asyncio.sleep( seconds )` – асинхронная задержка`

&nbsp;

## Задачи (Task), Future, корутины и Awaitable

**Задача (Task)** – обёртка над корутиной, которая создаётся когда корутина добавляется в очередь на обработку в цикле обработки событий. Например с помощью вызова `asyncio.create_task( )`.

Задачи также содержат в себе функции, которые будут вызванные когда корутина сможет продолжить выполняться.

Можно ожидать завершения задачи
...
Future – специальный тип, который представляет состояние запущенной задачи и способен сохранить внутри себя результат этого выполнения.

<img src="Awaitables.png" width=700>
Картинка из https://bbc.github.io/cloudfit-public-docs/asyncio/asyncio-part-2.html

# Применимость

| Задача    | Многопоточность | Multiprocessing | Asyncio        |
| --------- | --------------- | --------------- | -------------- |
| I/O bound | ✅ Хорошо       | ⚠️ Избыточно    | ✅✅ Идеально  |
| CPU bound | ❌ GIL          | ✅✅ Идеально   | ❌ Не подходит |
| Память    | Общая           | Копирование     | Общая          |

# Корутины в синхронном коде

Нельзя вызвать корутину вне цикла обработки событий.

Но можно сделать функцию-обёртку, которая уже сама по себе будет вызываться в синхронном коде и не будет зависеть от цикла обработки событий.
...
# функция обёртка для корутины
def sync_func(...):
    return asyncio.run(async_func(...))
    # запуск цикла обработки событий (run) - это блокирующая операция, т.к. цикл будет выполняться пока не закончатся события
```

При этом нужно проследить, чтобы цикл обработчики событий (asyncio.run) не вызывался внутри другого цикла обработки событий.

Другие варианты вызова асинхронных функций:

- создание специальных декораторов
- использование библиотечных функций обёрток, например

  ```py
  from asgiref.sync import async_to_sync

  result = async_to_sync(my_async_func)(*args)
  ```

В ряде случае имеет смысл создать две реализации функции (синхронную и асинхронную, т.е. корутину) или изначально создать только синхронные функции, и только потом на их основе создать корутину.
