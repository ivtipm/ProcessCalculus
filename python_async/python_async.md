...
2. Цикл по очереди обрабатывает *задачи* из очереди задач (Tast Queue)
3. Обработка задачи:
...
- **await** – ключевое слово, позволяющее приостановить выполнение корутины до получения результата асинхронной операции. `await` записывается перед *вызовом* функции. Например `res = await httpx.get( ... )`
