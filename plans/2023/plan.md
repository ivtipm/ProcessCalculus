...
- Установите Debian [на виртуальную машину]
   - Можно использовать минимальный образ, остальное скачать по сети во время установки: https://www.debian.org/CD/netinst/
   - Установите SSH server
   - Подключите физическую сетевую карту к гостевой ОС: Машина - Настроить - Сеть:\
     Адаптер: Тип подключения = Сетевой мост, ниже задайте устройство
     Теперь можно обращаться к виртуальной машине из локальной сети так, будто это обращение к хостовой (установленной на физическом устройстве) ОС


# Лекция 9. OpenMP
- Повторение
  - Как указать название используемой библиотеки при компиляции программы на C++?
...
#### Домашнее задание
1. Скачайте книгу по параллельному программированию для вашего любимого языка
1. Конспект. Потоки в вашем любимом языке программирования
...
Согласуйте язык программирования и задачу перед выполнением задания.

#### Задание (бонус). Брутфорс для веба
Подберите пароль для формы авторизации.

Создайте программу, которая использует несколько потоков (процессов для подбора пароля). Пароль состоит из 8 символов: 0-9, a-f.

См. логины в журнале заданий, в колонке с заданием WebBruteforce.

Адрес сервера см. в дискорде.

Если пароль подобран правильно, то сервер выдаст страницу со словаом "success".
...
# Лекция 5. Вычисления на GPU. Keras. Полносвязаня нейросеть.
...
#### Задание. Python Mind Map 

#### Задание. Virtual Enviroment
Отчёт по созданию виртуального окружения Python *(бонусное задание)*: зачем нужно? как сделать? Активировать? Деактивировать? На примере одной из своих программ и установки дополнительных пакетов.

Приведите пример установки зависимостей из файла requirements.txt
...
#### Задание. Numpy + matplotlib + seaborn + pandas
Python
   ...
   - ~~*дополнительно*: вычислите среднее значение элемента в матрицах реализованых через список и numpy array; сравните время вычисления~~ (см. сравнение list, numpy, torch выше)
