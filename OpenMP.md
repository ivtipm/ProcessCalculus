...
В секкции параллельного блока можно вызвать функцию получающую номер потока. В зависимости от номера потока выполнять ту или иную работу.
```C++
#include <omp.h>
...
#pragma omp parallel
{
    ...
    #pragma omp single nowait
    {
      ...
      while (node){
        ...
        // локальная (private) для потока переменная будет инициализирована (forst) значением глобальной переменной node
