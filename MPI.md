...
<img src="img/MPI_distributed_run.png" width=500>
...
- Основным средством коммуникации между процессами в MPI является передача сообщений друг другу, в общем случае по сети.
...
- Стандарт версии MPI 3.1 вышел в 2015 году.
- Стандарт MPI 4.0 вышел в 2021 году.
- Можно запускать в локальной сети или даже на локальном компьютере.
- Существует большое число реализаций, как бесплатных (MPICH, OpenMPI) так и коммерческих (Intel MPI).
  - **MPICH** разрабатывается в Аргоннской национальной лаборатории США, с участием компаний IBM, Cray, SiCortex, Microsoft, Intel, ...
    - Самая старая (до сиз пор обновляется) реализация
...
- Использовать виртуальную машину
  - Для изучения можно использовать VirtualBox, с ОС на базе Linux (например Debian);
  - Docker Container — узкоспециализированную виртуальную машину (запускается на Linux)\
  Чтобы виртуальная машина имела доступ к локальной сети можно настроить прямой доступ (bridge) гостевой ОС к сетевой карте.
- VDS
...
Любая программа должна в начале содержать вызов `MPI_Init(int* argc,     char*** argv)` — инициализацию среды выполнения. Все последующие функции MPI могут быть вызваны только после `MPI_Init`.
...
Тестовый запуск не MPI программы, например uptime:\
`mpiexec -n 2  uptime`
...
mpiexec и mpirun — синонимы.

После имени программы можно указать её параметры
`mpiexec -np 5 ./main 99999999`

Возможен одновременный запуск нескольких различных программ
...
После указания имени исполняемого файла для mpiexec можно передать параметры командной строки MPI программе.
...
Сообщения могут отправляться непосредственно адресату (большие сообщения) или записываться во временный буфер (маленькие сообщения). Тогда функция отправки сообщения может завершится раньше, чем функция приёма начнёт свою работу.


**Режимы передачи**
- **B**. с буферизацией. `MPI_Bsend`
...
1. Организовать VPN (https://averagelinuxuser.com/linux-vpn-server/)
  1. Установить OpenVPN на один из узлов\
    ...
    - в процессе установки указать (client name) имя конфигурационного файла (например `my-config`). Этот файл с расширением .ovpn будет использоваться для подключения клиентов к VPN серверу.
...
Коллективные операции *должны* вызываться *всеми* процессами.
...
`MPI_Bcast` — процесс с номером root рассылает сообщение из своего буфера передачи всем процессам области связи коммуникатора comm, остальные процессы принимают значение в аналогичный буфер.
...
См. также `MPI_Scatterv` —  в отличии от `MPI_Scatter` позволяет задавать число данных посылаемых каждому отдельному процессу.
