...
- запускаемая программа должна находится по одинаковому пути на всех узлах кластера (поэтому рекомендуется настроить NFS). Иначе ошибка `unable to change wdir to ...`
